...
function renderProductLoaded() {
  ...
  const selectedVolume = volumeOptions[state.selectedVolumeIndex] || volumeOptions[0];
  const price = formatPrice(selectedVolume.price);
...
function handleClick(event) {
  ...
  if (action === "add-to-cart") {
    const product = findRecommendedProduct(target.dataset.productId);
    addProductToCart(product, defaultProductVolume(product));
  ...
  if (action === "add-current-product") {
    const volumeOptions = productVolumeOptions(state.selectedProduct);
    addProductToCart(state.selectedProduct, volumeOptions[state.selectedVolumeIndex] || volumeOptions[0]);
    showToast("Аромат добавлен в корзину");
    renderProductLoaded();
  }
