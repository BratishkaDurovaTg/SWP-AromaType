...
const state = {
  ...
  cartItems: new Map(),
  // --- Никита: Новые поля для корзины и заказа ---
  cart: loadCart(),
  order: loadOrder(),
};
...
// --- Твоя функция renderProduct (НЕ ТРОГАЕМ! Сохраняем гендер, объёмы, дизайн) ---
    ...
    <section class="screen product-screen screen-with-footer">
      ...
      <div class="product-buy-layout">
        ...
        <div class="product-purchase">
          ...
          <button class="btn btn-small" data-action="add-current-product" type="button">Добавить<br>в корзину</button>
        </div>
      </div>
      ...
      <div class="bottom-actions">
        <button class="btn" data-action="add-current-product" type="button">Добавить в корзину</button>
      </div>
    </section>
  `);
}

// --- Твои вспомогательные функции (НЕ ТРОГАЕМ!) ---
...
function handleClick(event) {
  ...
  // --- Твой старый add-to-cart (оставляем для совместимости) ---
  if (action === "add-to-cart") {
    ...
    // Добавляем в старую и новую корзину
    addCartItem(product, volume);
    addProductToCart(product, volume);
  ...
  // --- Твой старый add-product-to-cart (оставляем) ---
  if (action === "add-product-to-cart") {
    addCartItem(state.selectedProduct, selectedProductVolume());
    addProductToCart(state.selectedProduct, selectedProductVolume());
    showToast("Аромат добавлен в корзину");
  }
  // --- Никита: Новый экшен для корзины ---
...
// --- Твои старые функции для корзины (оставляем для совместимости) ---
...
// --- Никита: Функции для новой корзины (добавлены поверх) ---
function addProductToCart(product, volume) {
  if (!product) {
    showToast("Не удалось добавить аромат");
    return;
  }
  const selectedVolume = normalizeVolumeSelection(product, volume);
  const normalizedVolume = selectedVolume.volumeMl;
  ...
  if (existing) {
    ...
    state.cart.push({
      ...
      brand: product.brand,
      imageUrl: product.imageUrl,
      price: numberValue(selectedVolume.price),
...
function productVolumeOptions(product) {
  const options = Array.isArray(product?.volumeOptions) ? product.volumeOptions : [];
  const normalized = options
    .map((option) => ({
      volumeMl: Number(option.volumeMl),
      price: numberValue(option.price),
    }))
    .filter((option) => SAMPLE_VOLUMES.includes(option.volumeMl));
  if (normalized.length) return normalized;
  return SAMPLE_VOLUMES.map((volumeMl) => ({
    volumeMl,
    price: numberValue(product?.price),
  }));
}

function defaultProductVolume(product) {
  return productVolumeOptions(product)[0];
}

function normalizeVolumeSelection(product, volume) {
  const volumeOptions = productVolumeOptions(product);

  if (typeof volume === "object" && volume !== null) {
    const volumeMl = Number(volume.volumeMl);
    const matched = volumeOptions.find((option) => option.volumeMl === volumeMl);
    if (matched) return matched;
  }

  const volumeMl = Number(volume);
  return volumeOptions.find((option) => option.volumeMl === volumeMl) || volumeOptions[0];
}
...
// --- Твои старые функции API и утилиты (НЕ ТРОГАЕМ!) ---
